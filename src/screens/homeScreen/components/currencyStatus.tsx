...
import { pairs,TradeContext } from '@/providers/tradeProvider';

const CurrencyList = (): JSX.Element => {
  const { currentPair, handleSwitchPair } = useContext(TradeContext);

  return (
    <Menu as='div' className='relative ml-3'>
      <div>
        <Menu.Button className='flex items-center gap-x-2 rounded p-2 text-xl font-extrabold hover:outline-none'>
...
const CurrencyStatus = () => {
  const { currentPairDetail } = React.useContext(TradeContext);
  return (
